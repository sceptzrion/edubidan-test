...
import { LecturerNotificationTab } from "@/components/dashboard/lecturer/settings/LecturerNotificationTab";
import { LecturerProfileTab } from "@/components/dashboard/lecturer/settings/LecturerProfileTab";
import { LecturerSecurityTab } from "@/components/dashboard/lecturer/settings/LecturerSecurityTab";

type LecturerSettingsTab = "profil" | "keamanan" | "notifikasi";

interface LecturerSettingsTabItem {
  id: LecturerSettingsTab;
  label: string;
  icon: LucideIcon;
}

const tabs: LecturerSettingsTabItem[] = [
...
export default function LecturerSettingsPage() {
  const [activeTab, setActiveTab] =
    useState<LecturerSettingsTab>("profil");
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
        ...
        <section className="flex-1 bg-card rounded-2xl sm:rounded-3xl border border-border p-5 sm:p-8 shadow-sm min-w-0">
          {activeTab === "profil" && <LecturerProfileTab />}
          {activeTab === "keamanan" && <LecturerSecurityTab />}
          {activeTab === "notifikasi" && <LecturerNotificationTab />}
        </section>
      </div>
    </div>
  );
